...
public int firstMissingPositive(int[] nums) {
    for (int i = 0; i < nums.length; i++) {
        /*
        	...
        	then  swap(nums, i, nums[i] - 1)  怎么来的？
        	如 nums = [3, 4, -1, 1]，原地哈希后，nums = [1, -1, 3, 4]，看元素 4：
        	3 的位置应该是 1（nums[0] = 1），但不是，所以把 3 放到正确的位置，即 nums[2]
        	if    nums[nums[1]-1] != nums[1]
        	then  swap(1, nums[1]-1)
        	
        	当前正确的条件：nums[nums[i]-1] != nums[i]
            这个条件检查的是：目标位置上的数字是否已经是我们要放的数字
            错误的条件：nums[i]-1 != i
            这个条件检查的是：当前数字是否已经在正确位置
        */
